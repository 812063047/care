...
export default {
    ...
    methods: {
      ...
      initCourse(){
        courseApi.getCourseListPage(1, 8,this.searchObj).then(response => {
      ...
      initSubject(){
        ...
        courseApi.getAllSubject().then(response => {
          this.subjectOneList = response.data.data.subjectList
        })
      },
      //3 分页切换的方法
    gotoPage(page) {
      courseApi.getCourseListPage(page,8,this.searchObj).then(response => {
        this.data = response.data.data
      })
    },
